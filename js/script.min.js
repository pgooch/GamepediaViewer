...
var _indexedDBDetails = {
	...
	'lastUpdated': 958867200000 - _pediaviewerConst.macEpochOffset };
		...
		switch (cleanedHash) {
			...
			case 'settings':
				...
				updateCache(function () {
					switch (cleanedHash) {
						case '':
							ReactDOM.render(React.createElement(MainView, null), document.getElementById('container'));break;
						case 'title':
							ReactDOM.render(React.createElement(DetailView, null), document.getElementById('container'));break;
						case 'settings':
							ReactDOM.render(React.createElement(SettingsView, null), document.getElementById('container'));break;
					}
				}.bind(cleanedHash));
				break;
...
*/
var updateCache = function updateCache(callback) {
	...
	render: function render() {
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(List, { filters: this.state.filters })
			),
			...
			if (filters == JSON.stringify(this.props.filters)) {
				...
				if (e.target.result == null) {
					...
					if (_listContents.length <= 0) {
						_listContents.push(React.createElement(
							'li',
							{ key: 'no-results-message', className: 'no-results' },
...
var DetailView = React.createClass({
	...
	componentWillMount: function componentWillMount() {
		...
		objectStoreRequest.onsuccess = function (e) {
			...
			Object.keys(details).map(function (key) {
				...
				fieldTemp.push(React.createElement(
					'li',
	...
	render: function render() {
		return React.createElement(
			...
			React.createElement(DetailCoverImage, { titleID: parseInt(window.location.hash.match(/^#title-(\d+)-/)[1]) })
		);
	},
	componentDidMount: componentDidMountGlobal
});
	...
	render: function render() {
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					'a',
					{ href: _extraDetails.latestTitle, className: 'footer-link' },
					'Latest Entry'
				)
			),
			React.createElement(
